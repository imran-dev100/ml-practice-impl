...
def plt_stationary(x_train, y_train):
    ...
    cscat  = ax[1].scatter(w0,b, s=100, color='blue', zorder= 10, label="cost with \ncurrent w,b")
    chline = ax[1].hlines(b, ax[1].get_xlim()[0],w0, lw=4, color='purple', ls='dotted')
    cvline = ax[1].vlines(w0, ax[1].get_ylim()[0],b, lw=4, color='purple', ls='dotted')
    ...
    ax[2].plot_surface(tmp_w, tmp_b, z,  cmap = 'ocean', alpha=0.3, antialiased=True)
